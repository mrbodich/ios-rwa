...
extension View {
    @ViewBuilder
    func onNavigation(wrapped: Bool = true, _ action: @escaping () -> Void) -> some View {
        ...
        if wrapped {
            NavigationLink( destination: EmptyView(), isActive: isActive ) { self }
        }
        else {
            self
        }
    ...
    func navigation<Destination: View>(
        ...
        overlay(
            NavigationLink(destination: isActive.wrappedValue ? destination() : nil, isActive: isActive, label: { EmptyView() })
        )
    }
